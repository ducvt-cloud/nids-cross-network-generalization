# ============================================================================
# DOMAIN ADAPTATION (Deep CORAL) — kiểm dự đoán label-conditional
#   Tự chứa: tải lại dữ liệu, căn chỉnh, giải nén ToN.
#   Với mỗi cặp source->target: so AUROC BASELINE (source-only) vs CORAL
#   (căn chỉnh hiệp phương sai đặc trưng source/target, dùng X đích KHÔNG nhãn).
#   Dự đoán cần kiểm: vì p(y|x) ĐẢO giữa miền, CORAL (căn chỉnh p(x)) sẽ
#   KHÔNG cứu cặp đảo, có thể giúp cặp đồng-hướng/dịch nhẹ.
# ============================================================================
...
for n in list(DATASETS):
    ...
    if r is None: print(f"  ⚠️ Bỏ {SHORT[n]} ({SRC[n]})"); DATASETS.remove(n)
    else: raw[n]=r
...
# ------------------------------ RUN ------------------------------
print("\n"+"="*78)
print("BASELINE (source-only)  vs  CORAL  — AUROC trên test của TARGET")
print("="*78)
rows=[]
for s in DATASETS:
    ...
    for t in DATASETS:
        if t==s: continue
        Xt_tr=to_f32(sc.transform(SP[t]['Xtr']))      # X đích không nhãn (để CORAL căn chỉnh)
        ...
        kind = 'ĐỒNG-HƯỚNG' if b>=0.65 else ('ĐẢO' if b<0.45 else 'gần ngẫu nhiên')
        rows.append((s,t,b,co,kind))
        print(f"  {SHORT[s]:>4} -> {SHORT[t]:<4} | base={b:.3f}  coral={co:.3f}  Δ={co-b:+.3f}  [{kind}]")

# tóm tắt theo loại cặp
print("\n"+"-"*78)
for k in ['ĐỒNG-HƯỚNG','gần ngẫu nhiên','ĐẢO']:
    sub=[(co-b) for (_,_,b,co,kk) in rows if kk==k]
    if sub: print(f"  Cặp {k:<14}: n={len(sub):>2} | Δ trung bình (CORAL - base) = {np.mean(sub):+.3f}")
print("\nDONE.")
print("Dự đoán: CORAL ~0 hoặc xấu đi ở cặp ĐẢO (p(y|x) đảo, căn chỉnh p(x) không cứu);")
print("         có thể dương ở cặp đồng-hướng/dịch nhẹ. Nếu đúng => 'UDA không đủ, cần few-shot/nhãn đích'.")
