# ============================================================================
# IN-DOMAIN LEAKAGE CHECK: random per-flow split  vs  GROUP by source-IP
#   Question: is the in-domain ~0.99 inflated by host correlation (the same host
#   appearing in both train and test)? IP is NOT a feature (dropped), but flows
#   from the same host are still correlated -> a random split can leak.
#   Method: for each dataset, measure in-domain under (a) random split, (b) grouping by
#   IPV4_SRC_ADDR (StratifiedGroupKFold, no host on both sides).
#   If grouped << random -> host leakage; if ~ equal -> the high in-domain is GENUINE.
...
def load_with_ip(name,max_rows,chunk=500_000):
    """Return X (NUMERIC features, IP REMOVED), y, and group=source-IP (for the grouped split)."""
    ...
    groups=df[ipc].astype(str).values if ipc else np.arange(len(df))   # if no IP -> each flow is its own group (=random)
...
for n in DATASETS:
    ...
    if r is None: print(f"  WARN: skipping {SHORT[n]}"); continue
    X,y,groups,ng=r
...
print("\nDONE.")
print("Read: grouped ≈ random (~0.99) => the high in-domain is GENUINE, not host memorization (IP removed from features).")
print("     grouped << random => host-correlation leakage -> in-domain inflated (also a useful finding).")
print("     'groups' = number of distinct source-IPs; if too few, the grouped split is unstable (interpret with caution).")
