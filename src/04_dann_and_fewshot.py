# ============================================================================
# CELL #3 — METHOD: DANN (UDA thứ hai) + FEW-SHOT, có AUROC & AUPRC
#   (A) DANN (domain-adversarial) vs baseline trên các cặp ĐẢO -> kiểm "UDA
#       không-giám-sát không đủ" có đúng không chỉ riêng CORAL.
#   (B) Few-shot: AUROC & AUPRC theo k nhãn đích/lớp (k=0,10,50,100) + ceiling.
...
# ---------------- RUN (A) DANN ----------------
print("\n"+"="*78+"\n(A) DANN vs BASELINE trên cặp ĐẢO — AUROC | AUPRC (base-rate đích)\n"+"="*78)
...
# ---------------- RUN (B) few-shot ----------------
print("\n"+"="*78+"\n(B) FEW-SHOT — AUROC / AUPRC theo k (số nhãn đích/lớp)\n"+"="*78)
ceil={}
for t in DATASETS:
    sc=QuantileTransformer(output_distribution='uniform',n_quantiles=1000,random_state=SEED)
...
print("\nDONE.  (định dạng: AUROC/AUPRC)")
print("(A) Nếu DANN cũng KHÔNG kéo cặp đảo lên (như CORAL) => 'UDA không-giám-sát không đủ' vững (không riêng CORAL).")
print("(B) Xem cần k bao nhiêu để AUROC&AUPRC vượt ngưỡng hữu dụng; AUPRC đọc kèm base-rate đích.")
