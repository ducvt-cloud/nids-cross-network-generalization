# ============================================================================
# CELL 1 — ADAPTATION với 5 SEED + STD  (cho §5.4: CORAL, DANN, few-shot)
#   Part A: baseline vs CORAL vs DANN — AUROC mean±std + Δ, trên các cặp đại diện.
#   Part B: few-shot — AUROC/AUPRC mean±std theo k={0,10,50,100} + ceiling.
#   In kết quả TĂNG DẦN theo từng cặp -> đứt giữa chừng vẫn còn số đã chạy.
...
# 6 cặp đại diện: 3 đảo + 1 gần-ngẫu-nhiên + 1 đồng-hướng + 1 CORAL-từng-cứu
...
def train_mlp(Xs,ys,Xf,yf,seed):     # baseline (Xf rỗng) hoặc few-shot (trộn nhãn đích)
...
# ============ PART A: baseline vs CORAL vs DANN (5 seed, mean±std) ============
print("\n"+"="*92)
print(f"PART A — BASELINE vs CORAL vs DANN | {N_SEEDS} seed, AUROC mean±std (base-rate đích)")
print("="*92)
for s,t in PAIRS:
...
# ============ PART B: few-shot (5 seed, mean±std) ============
print("\n"+"="*92)
print(f"PART B — FEW-SHOT | {N_SEEDS} seed | mỗi ô = AUROC(mean±std) / AUPRC(mean±std)")
print("="*92)
ceil={}
for t in DATASETS:
    sc=QuantileTransformer(output_distribution='uniform',n_quantiles=1000,random_state=SEED)
...
print("\nDONE. (Part A nâng số CORAL/DANN; Part B nâng Bảng 6 — đều có ±std qua 5 seed.)")
