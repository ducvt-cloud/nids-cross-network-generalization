# ============================================================================
# MA TRẬN TRANSFER "BẢN CUỐI" — RF+LGBM+MLP, 5 seed, AUROC + AUPRC + base-rate
#   - In ma trận AUROC (mean±std) và ma trận AUPRC (mean±std) cho TỪNG mô hình.
#   - In base-rate (attack%) của mỗi target -> để diễn giải AUPRC đúng
#     (AUPRC chỉ có nghĩa khi attack là THIỂU SỐ; ở BoT 99.6% attack thì AUPRC
#      bị base-rate thổi, phải đọc kèm).
...
# ------------------------------ RUN ------------------------------
print("\nbase-rate (attack% test) mỗi bộ:  "+" | ".join(f"{SHORT[n]}={BASE[n]*100:.1f}%" for n in DATASETS))
for kind in ['RF','LGBM','MLP']:
    print("\n"+"="*88+f"\nMÔ HÌNH: {kind}  | {N_SEEDS} seed (hàng=train, cột=test)\n"+"="*88)
    ro,pr=run_family(kind)
    print_mat("AUROC (mean±std):",ro)
    print_mat("AUPRC (mean±std):",pr)

print("\nDONE.")
print("Đọc: AUROC = chỉ số chính (so xuyên-miền công bằng). AUPRC đọc KÈM base-rate:")
print("  - chỉ diễn giải khi attack là thiểu số (CSE 12%, UNSW 4%, ToN 64%).")
print("  - ở BoT (attack 99.6%) AUPRC nền đã ~0.996 -> KHÔNG dùng AUPRC cho cột BoT.")
print("  - so AUPRC giữa các bộ phải nhớ sàn ngẫu nhiên = base-rate của bộ đó.")
