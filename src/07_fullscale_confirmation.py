# ============================================================================
# CELL 3 — XÁC NHẬN QUY MÔ LỚN  (collapse có bền ở quy mô lớn không?)
#   Chạy lại MA TRẬN TRANSFER AUROC 4x4 ở ~1,000,000 dòng/bộ (gấp ~7x so 150K),
#   bằng LightGBM (nhẹ RAM, nhanh trên dữ liệu lớn). 3 seed (split khác nhau) -> mean±std.
#   So sánh trực tiếp với Bảng 3 (150K) để loại khả năng "artifact do subsample".
#   Tự chứa: tải lại, giải nén ToN. Chỉnh MAX_ROWS_PER_DS nếu RAM cho phép.
# ============================================================================
...
print(f"Đang nạp pool tới {MAX_ROWS_PER_DS:,} dòng/bộ (có thể vài phút)...")
POOL={}
for n in list(DATASETS):
    ...
    if r is None: print(f"  ⚠️ Bỏ {SHORT[n]}"); DATASETS.remove(n)
        ...
        POOL[n]=r; print(f"  {SHORT[n]}: {len(r[1]):,} dòng | attack-rate={r[1].mean():.3f}")
...
# ma trận AUROC (và AUPRC) mean±std qua N_SEEDS
import numpy as np
...
for sd in range(N_SEEDS):
    ...
    for s in DATASETS:
        ...
        print(f"  {SHORT[s]:>4} đã train & chấm 4 đích")
        del m; gc.collect()
    del spl; gc.collect()

def ms(v): a=np.array(v,float); return np.nanmean(a),np.nanstd(a)
print("\n"+"="*78)
print(f"MA TRẬN AUROC ở QUY MÔ LỚN (~{MAX_ROWS_PER_DS:,}/bộ, LGBM, {N_SEEDS} seed) — mean±std")
print("hàng=train(nguồn), cột=test(đích). Đối chiếu Bảng 3 (150K).")
print("="*78)
...
for s in DATASETS:
    ...
    print(row)
print(f"\nbase-rate đích: "+" ".join(f"{SHORT[t]}={base[t]:.3f}" for t in DATASETS))
print("\nMA TRẬN AUPRC (mean) — đọc kèm base-rate; bỏ cột BoT (artifact):")
...
for s in DATASETS:
    ...
    print(row)
print("\nDONE. Nếu off-diagonal vẫn collapse/đảo như Bảng 3 -> kết luận bền ở quy mô lớn.")
print("Muốn tiến sát 'full', tăng MAX_ROWS_PER_DS (chú ý RAM); BoT/CSE rất lớn.")
